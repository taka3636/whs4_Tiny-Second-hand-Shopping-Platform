...
## 2. 설치 및 실행 (다른 환경에서 실행하는 방법)

GitHub 저장소를 받은 뒤 아래 순서대로 실행하면 됩니다. (git, Node.js 22.5+ 만 있으면 됩니다.)
...
nvm install && nvm use
node -v                 # v22.5 이상인지 확인

# 3) 의존성 설치 (node_modules 는 저장소에 없으므로 반드시 실행)
npm install
...
- (선택) 외부에서 접속시키려면 `ngrok http 3000` 으로 발급된 주소를 사용합니다.

**자주 겪는 문제**
- `Cannot find module ...` → `npm install`을 안 했거나 `app` 폴더가 아님 (`cd app` 후 `npm install`)
- `node:sqlite` 관련 에러 / 실행 안 됨 → `node -v`가 22.5 미만 (`nvm install 22 && nvm use 22`)
